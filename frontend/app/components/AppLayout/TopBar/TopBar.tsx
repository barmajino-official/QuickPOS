/**
 * @file TopBar.tsx
 * @description Sticky page header: mobile menu button, page title, live clock,
 *              theme toggle, and an optional actions slot (e.g. "+ New" buttons).
 *
 * @receives title   – the current page title
 * @receives actions – optional node rendered on the right (page-level actions)
 *
 * @notes The menu button only shows below `md` and toggles the sidebar drawer via
 *        uiStore. The clock hides on the smallest screens to save room.
 */
import { useEffect, useState } from 'react';

import { useUiStore } from '~/store/uiStore';

interface Props {
  title: string;
  actions?: React.ReactNode;
}

const CLOCK_TICK_MS = 1000;

export function TopBar({ title, actions }: Props) {
  const [time, setTime] = useState(new Date());
  const toggleSidebar = useUiStore((s) => s.toggleSidebar);
  const [isDark, setIsDark] = useState(false);

  useEffect(() => {
    const timer = setInterval(() => setTime(new Date()), CLOCK_TICK_MS);

    const currentTheme = document.documentElement.getAttribute('data-theme') || localStorage.getItem('pos_theme') || 'montajat';
    setIsDark(currentTheme === 'montajat-dark');

    return () => clearInterval(timer);
  }, []);

  const handleToggleTheme = (e: React.ChangeEvent<HTMLInputElement>) => {
    const nextTheme = e.target.checked ? 'montajat-dark' : 'montajat';
    document.documentElement.setAttribute('data-theme', nextTheme);
    localStorage.setItem('pos_theme', nextTheme);
    setIsDark(e.target.checked);
  };

  const dateLabel = time.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
  const timeLabel = time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' });

  return (
    <header className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3.5 bg-[var(--color-base-100)] sticky top-0 z-30 m3-elevation-1">
      <div className="flex items-center gap-3 min-w-0">
        <button className="md:hidden flex items-center justify-center w-10 h-10 rounded-full text-[var(--color-base-content)] flex-shrink-0 hover:bg-[color-mix(in_srgb,var(--color-base-300)_40%,transparent)] [transition:background-color_0.2s_var(--m3-standard)]" onClick={toggleSidebar} aria-label="Open menu">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
            <line x1="4" y1="7" x2="20" y2="7" />
            <line x1="4" y1="12" x2="20" y2="12" />
            <line x1="4" y1="17" x2="20" y2="17" />
          </svg>
        </button>
        <h1 className="text-lg sm:text-xl font-medium text-[var(--color-base-content)] truncate">{title}</h1>
      </div>

      <div className="flex items-center gap-3 sm:gap-5 flex-shrink-0">
        {actions && <div className="flex items-center gap-2">{actions}</div>}

        <div className="hidden sm:flex flex-col items-end leading-tight">
          <span className="text-sm font-medium text-[var(--color-base-content)]">{timeLabel}</span>
          <span className="text-xs text-[var(--color-neutral)]">{dateLabel}</span>
        </div>

        <label className="swap swap-rotate text-[var(--color-base-content)] hover:text-[var(--color-primary)] [transition:color_0.2s_var(--m3-standard)]">
          <input type="checkbox" checked={isDark} onChange={handleToggleTheme} />
          <svg className="swap-off fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" /></svg>
          <svg className="swap-on fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" /></svg>
        </label>
      </div>
    </header>
  );
}
