...
import {
  IconDashboard, IconPOS, IconOrders, IconProducts,
  IconCategories, IconCustomers, IconStaff, IconSun, IconMoon,
} from "./components/Icons";

const NAV_ITEMS = [
  { path: "/dashboard",  id: "dashboard",  label: "Dashboard",        icon: IconDashboard  },
  ...
  { path: "/categories", id: "categories", label: "Categories",       icon: IconCategories },
  { path: "/customers",  id: "customers",  label: "Customers",        icon: IconCustomers  },
...
function ProtectedRoute({ permKey, children }: { permKey: keyof Permissions; children: ReactNode }) {
  const { permissions } = useAuth();
  if (!permissions?.[permKey]) {
    const first = NAV_ITEMS.find(i => permissions?.[i.id as keyof Permissions]);
...
function AppShell() {
  ...
  const visibleNav  = NAV_ITEMS.filter(i => permissions?.[i.id as keyof Permissions]);
  const isActive    = (path: string) => location.pathname === path || (path === "/dashboard" && location.pathname === "/");
  const pageLabel   = NAV_ITEMS.find(i => i.path === location.pathname)?.label
    ?? (location.pathname === "/profile" ? "My Profile" : "");
  const defaultPath = permissions?.dashboard ? "/dashboard" : (visibleNav[0]?.path ?? "/profile");
    ...
    <div className="drawer lg:drawer-open bg-base-200 min-h-screen font-sans">
      ...
      <div className="drawer-content flex flex-col h-screen overflow-hidden">
        ...
        <div className="navbar bg-base-100 border-b border-base-300 sticky top-0 z-30 px-6 py-2 shadow-sm min-h-0 h-14">
          <label htmlFor="sidebar" className="btn btn-ghost btn-sm btn-square lg:hidden mr-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" className="h-5 w-5 stroke-current text-base-content">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </label>

          <div className="flex-1 lg:hidden text-lg font-bold text-base-content tracking-tight">QuickPOS</div>
          ...
          <div className="flex-1 hidden lg:flex items-center text-sm font-medium opacity-60 text-base-content gap-4">
            ...
            <span className="font-mono text-xs font-bold">
              {time.toLocaleString("en-US", { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit" })}
            </span>
          </div>
          ...
          <div className="flex-none gap-3">
            ...
            <div className="dropdown dropdown-end">
              <div tabIndex={0} role="button" className="btn btn-ghost btn-sm px-2 gap-2 hover:bg-base-200 transition-colors border border-base-300 rounded-md text-base-content">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-50">
                  <path d="m6 9 6 6 6-6" />
                </svg>
              </div>
              <ul tabIndex={0} className="menu menu-sm dropdown-content mt-2 z-1 p-1 shadow-md bg-base-100 rounded-md w-48 border border-base-300">
                <li>
                  <Link to="/profile" className="rounded-sm py-2 text-base-content font-medium">
                    Account Profile
                  </Link>
                </li>
                <li>
                  <button onClick={signOut} className="text-error rounded-sm py-2 mt-1 hover:bg-error/10 font-medium">
                    Sign Out
                  </button>
                </li>
              </ul>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Routes location={location}>
                ...
                <Route path="/dashboard"   element={<ProtectedRoute permKey="dashboard"><Dashboard /></ProtectedRoute>} />
                <Route path="/pos"         element={<ProtectedRoute permKey="pos"><POS staffId={session!.user.id} /></ProtectedRoute>} />
                <Route path="/orders"      element={<ProtectedRoute permKey="orders"><Orders /></ProtectedRoute>} />
                ...
                <Route path="/staff"       element={<ProtectedRoute permKey="staff"><StaffPage /></ProtectedRoute>} />
        ...
        <aside className="bg-base-100 text-base-content w-64 h-full flex flex-col border-r border-base-300">
          <div className="h-14 flex items-center px-6 border-b border-base-300 bg-base-200/30">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-base font-bold tracking-tight text-base-content">
                QuickPOS <span className="opacity-50 text-xs font-normal ml-1">Pro</span>
              </h1>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto py-4">
            ...
            <ul className="px-2 space-y-0.5">
              {visibleNav.length === 0 && (
                <div className="px-4 py-2 text-xs opacity-50 italic">No modules available</div>
              )}
              {visibleNav.map(item => {
                ...
                return (
                  <li key={item.path}>
                    <Link to={item.path} className="block group">
                      <div className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors text-sm ${active ? "bg-primary/10 text-primary font-bold" : "hover:bg-base-200 text-base-content/80 font-medium"}`}>
                        <span className={`opacity-80 text-base ${active ? "text-primary" : "grayscale group-hover:grayscale-0"}`}>
                          {item.icon}
                        </span>
                        {item.label}
                      </div>
